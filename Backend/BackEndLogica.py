...
for index, row in data.iterrows():
    ...
    if type(anime_title) != float:
        animeNames.append(row['titleEnglish'])
    else:
        animeNames.append(row['titleRomaji'])
    animePictures.append(row['image'].replace("\\", "/"))
    frontEndData += "\n\t{"
    for attribute in attributes:
        if type(row[attribute]) == str:
            frontEndData += f"{attribute}: \"{row[attribute]}\","
        else:
            frontEndData += f"{attribute}: {row[attribute]},"
    frontEndData += "},"
    if type(anime_title) == float:
